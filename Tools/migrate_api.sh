...
if [ ! -f "$PLAYPATH/framework/play-1.2.4.jar" ]; then
...
cd $CLASSDIR
CLASSPATH=$PLAYPATH/framework/play-1.2.4.jar:$PLAYPATH/framework/lib/gson-1.7.1.jar javac `find $JAVADSTDIR/ -name "*.java"` -d .
if [ $? -ne 0 ]; then
